...
class FormRenderer {
	...
	public static function notice( string $source, string $type ): void {
		...
		if ( $ok && ! $msg ) {
			$msg = __( 'Thank you - your submission has been received and will be reviewed.', 'seedcast-bulletin-library' );
		}
		printf(
	...
	public static function open( string $source, string $type, array $atts = [] ): void {
		...
		if ( get_option( 'seedcast_honeypot_enabled', '1' ) === '1' ) {
			...
			echo '<label>' . esc_html__( 'Leave this field empty', 'seedcast-bulletin-library' ) . '</label>';
	...
	public static function author_fields( bool $require_name = false ): void {
		...
		<div class="sc-form__row">
			<label class="sc-form__label" for="seedcast_author_name">
				<?php esc_html_e( 'Your name', 'seedcast-bulletin-library' ); ?>
				<?php if ( $require_name ) : ?>
					<span class="sc-form__req" aria-hidden="true">*</span>
					<span class="sc-screen-reader-text"><?php esc_html_e( '(required)', 'seedcast-bulletin-library' ); ?></span>
				<?php endif; ?>
			</label>
			<input type="text" id="seedcast_author_name" name="seedcast_author_name" <?php echo $require_name ? 'required' : ''; ?> />
			<p class="sc-form__help"><?php esc_html_e( 'How you would like to be credited. You can choose to stay anonymous below.', 'seedcast-bulletin-library' ); ?></p>
		</div>
		<div class="sc-form__row">
			<label class="sc-form__label" for="seedcast_author_email"><?php esc_html_e( 'Email (optional)', 'seedcast-bulletin-library' ); ?></label>
			<input type="email" id="seedcast_author_email" name="seedcast_author_email" />
			<p class="sc-form__help"><?php esc_html_e( 'Kept private. Only used if we need to follow up with you.', 'seedcast-bulletin-library' ); ?></p>
		</div>
		<?php
	}
