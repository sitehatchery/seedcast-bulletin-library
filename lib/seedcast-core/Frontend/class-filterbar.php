...
class FilterBar {
	...
	public static function pills( array $items, array $args = [] ): void {
		$args = wp_parse_args( $args, [
			'all_label'  => __( 'All', 'seedcast-bulletin-library' ),
			'all_url'    => '',
			'all_active' => false,
			'aria'       => __( 'Filter', 'seedcast-bulletin-library' ),
		] );
	...
	public static function search( array $args = [] ): void {
		$args = wp_parse_args( $args, [
			...
			'param'       => 's',
			'value'       => '',
			'placeholder' => __( 'Search…', 'seedcast-bulletin-library' ),
			'hidden'      => [],
		] );
		...
		echo '<button type="submit" class="sc-btn">' . esc_html__( 'Search', 'seedcast-bulletin-library' ) . '</button>';
		echo '</form>';
	}
}
