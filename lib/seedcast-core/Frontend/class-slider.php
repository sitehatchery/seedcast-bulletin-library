...
class Slider {
	...
	public static function render( array $cards, array $args = [] ): void {
		$args = wp_parse_args( $args, [
			'label' => __( 'Carousel', 'seedcast-bulletin-library' ),
			'empty' => __( 'Nothing to show yet.', 'seedcast-bulletin-library' ),
		] );
	...
	public static function open( string $label = '' ): void {
		...
		echo '<button class="sc-slider__nav sc-slider__nav--prev" aria-label="' . esc_attr__( 'Previous', 'seedcast-bulletin-library' ) . '">&#8592;</button>';
		echo '<div class="sc-slider__track" role="region" aria-label="' . esc_attr( $label ) . '">';
	}

	public static function close(): void {
		echo '</div>'; // .sc-slider__track
		echo '<button class="sc-slider__nav sc-slider__nav--next" aria-label="' . esc_attr__( 'Next', 'seedcast-bulletin-library' ) . '">&#8594;</button>';
		echo '</div>'; // .sc-slider
	}
}
