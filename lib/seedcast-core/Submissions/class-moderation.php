...
class Moderation {
	...
	public function ajax_moderate(): void {
		...
		if ( ! current_user_can( 'edit_posts' ) ) {
			wp_send_json_error( [ 'message' => __( 'Permission denied.', 'seedcast-bulletin-library' ) ], 403 );
		}
		...
		if ( ! $id || ! self::set_status( $id, $status, $notes ) ) {
			wp_send_json_error( [ 'message' => __( 'Could not update submission.', 'seedcast-bulletin-library' ) ] );
		}
