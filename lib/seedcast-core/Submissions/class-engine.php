...
class SubmissionEngine {
	...
	private function process( array $post ): array {
		...
		if ( ! isset( $post['seedcast_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( $post['seedcast_nonce'] ), $nonce_action ) ) {
			return [ false, __( 'Security check failed. Please try again.', 'seedcast-bulletin-library' ) ];
		}
		...
		if ( ! Captcha::passes_honeypot() ) {
			...
			return [ false, __( 'Your submission could not be verified.', 'seedcast-bulletin-library' ) ];
		}
		if ( ! Captcha::verify() ) {
			return [ false, __( 'Your submission could not be verified.', 'seedcast-bulletin-library' ) ];
		}
	...
	public function ajax_submit(): void {
		...
		if ( $ok ) {
			...
			$payload = apply_filters(
				...
				[ 'message' => __( 'Thank you - your submission has been received and will be reviewed.', 'seedcast-bulletin-library' ) ],
				$post
			);
	...
	public function insert( array $data ) {
		...
		if ( empty( $data['source'] ) || empty( $data['type'] ) ) {
			return new \WP_Error( 'seedcast_missing_source', __( 'Submission source and type are required.', 'seedcast-bulletin-library' ) );
		}
		...
		if ( false === $ok ) {
			...
			return new \WP_Error( 'seedcast_db_error', __( 'Could not save submission.', 'seedcast-bulletin-library' ) );
		}
