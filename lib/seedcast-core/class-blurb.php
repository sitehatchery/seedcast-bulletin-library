...
final class Blurb {
	...
	public static function sermon( array $args = [] ): string {
		$args = array_merge(
			[
				...
				'date'    => '',
				'series'  => '',
				'subject' => __( 'this message', 'seedcast-bulletin-library' ),
			],
			$args
		);
		...
		if ( '' !== trim( (string) $args['series'] ) ) {
			$sentences[] = sprintf(
				...
				__( 'It was part of the %s series.', 'seedcast-bulletin-library' ),
	...
	private static function opening( array $args ): string {
		...
		$where = '' !== $locality
			? sprintf(
				...
				__( '%1$s in %2$s', 'seedcast-bulletin-library' ),
				$church,
				$locality
			)
			: $church;
		...
		if ( '' !== $speaker && '' !== $date ) {
			return sprintf(
				...
				__( '%1$s preached %2$s at %3$s on %4$s.', 'seedcast-bulletin-library' ),
				$speaker,
				$subject,
				$where,
				$date
		...
		if ( '' !== $speaker ) {
			return sprintf(
				...
				__( '%1$s preached %2$s at %3$s.', 'seedcast-bulletin-library' ),
				$speaker,
				$subject,
				$where
			);
		...
		if ( '' !== $date ) {
			return sprintf(
				...
				__( '%1$s was preached at %2$s on %3$s.', 'seedcast-bulletin-library' ),
				ucfirst( $subject ),
				$where,
				$date
			);
		...
		return sprintf(
			...
			__( '%1$s was preached at %2$s.', 'seedcast-bulletin-library' ),
			ucfirst( $subject ),
			$where
		);
	}
	...
	private static function gathering(): string {
		...
		return sprintf(
			...
			__( 'We gather %1$s at %2$s.', 'seedcast-bulletin-library' ),
			self::plural_day( $day ),
			self::join( $times )
		);
	}
	...
	private static function join( array $items ): string {
		...
		return sprintf(
			...
			__( '%1$s and %2$s', 'seedcast-bulletin-library' ),
			implode( ', ', $items ),
			$last
		);
	}
