...
final class SuiteGrid {
	...
	public static function catalog(): array {
		return array(
			array(
				...
				'tagline' => __( 'The bulletin, alive online. Weekly service pages that gather programs, announcements, and handouts.', 'seedcast-bulletin-library' ),
			...
			array(
				...
				'channel' => 'free',
				'name'    => 'Sermon Library',
				'tagline' => __( 'Sermons with series, speakers, scripture, and a podcast feed.', 'seedcast-bulletin-library' ),
			...
			array(
				...
				'tagline' => __( 'Transcription, summaries, and automatic content preparation for the plugins you already run.', 'seedcast-bulletin-library' ),
	...
	public static function render(): void {
		...
		?>
		<p class="description">
			<?php esc_html_e( 'Seedcast plugins share one set of settings and one look. Anything you install later inherits both automatically.', 'seedcast-bulletin-library' ); ?>
		</p>
			...
			<div class="sc-tile__foot">
				<?php if ( $is_pro ) : ?>
					<a class="sc-tile__link" href="<?php echo esc_url( $card['url'] ); ?>" target="_blank" rel="noopener">
						<?php esc_html_e( 'Learn more', 'seedcast-bulletin-library' ); ?>
					</a>
					<span class="sc-tile__status"><?php esc_html_e( 'Add-on', 'seedcast-bulletin-library' ); ?></span>
				<?php elseif ( $card['active'] ) : ?>
					<a class="sc-tile__link" href="<?php echo esc_url( admin_url( $card['admin'] ) ); ?>">
						<?php esc_html_e( 'Open', 'seedcast-bulletin-library' ); ?>
					</a>
					<span class="sc-tile__status"><?php esc_html_e( 'Active', 'seedcast-bulletin-library' ); ?></span>
				<?php elseif ( $card['installed'] ) : ?>
					<?php if ( current_user_can( 'activate_plugins' ) ) : ?>
						<button type="button" class="button button-small sc-tile__activate">
							<?php esc_html_e( 'Activate', 'seedcast-bulletin-library' ); ?>
						</button>
					<?php endif; ?>
					<span class="sc-tile__status"><?php esc_html_e( 'Inactive', 'seedcast-bulletin-library' ); ?></span>
				<?php else : ?>
					<?php if ( current_user_can( 'install_plugins' ) ) : ?>
						<button type="button" class="button button-small sc-tile__install">
							<?php esc_html_e( 'Install', 'seedcast-bulletin-library' ); ?>
						</button>
					<?php endif; ?>
					<span class="sc-tile__status"><?php esc_html_e( 'Not installed', 'seedcast-bulletin-library' ); ?></span>
				<?php endif; ?>
			</div>
		</div>
		<?php
	...
	public function ajax_activate(): void {
		...
		if ( ! current_user_can( 'activate_plugins' ) ) {
			wp_send_json_error( array( 'message' => __( 'Permission denied.', 'seedcast-bulletin-library' ) ), 403 );
		}

		$slug  = isset( $_POST['slug'] ) ? sanitize_key( wp_unslash( $_POST['slug'] ) ) : '';
		$entry = self::entry( $slug );
		if ( ! $entry ) {
			wp_send_json_error( array( 'message' => __( 'Not a Seedcast plugin.', 'seedcast-bulletin-library' ) ), 400 );
		}
	...
	public function ajax_install(): void {
		...
		if ( true !== $result ) {
			wp_send_json_error( array( 'message' => __( 'Install did not complete.', 'seedcast-bulletin-library' ) ), 500 );
		}
