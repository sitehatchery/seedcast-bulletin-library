...
final class CompletenessReport {
	...
	public static function render( string $source ): void {
		...
		if ( null === $definition ) {
			echo '<div class="notice notice-error"><p>'
				. esc_html__( 'No completeness definition is registered for this content.', 'seedcast-bulletin-library' )
				. '</p></div>';
			return;
		}
	...
	private static function render_items( string $source, array $definition ): void {
		...
		<div class="sc-completeness">
			<h2><?php esc_html_e( 'What is missing', 'seedcast-bulletin-library' ); ?></h2>
			<p class="sc-completeness__lead">
				<?php esc_html_e( 'How many items are missing each field. The one at the top is where filling something in moves your score the most.', 'seedcast-bulletin-library' ); ?>
			</p>
				...
				<?php foreach ( $gaps as $label => $count ) : ?>
					<li class="sc-gap">
						...
						<span class="sc-gap__count <?php echo 0 === (int) $count ? 'sc-gap__count--none' : ''; ?>">
							<?php
							if ( 0 === (int) $count ) {
								esc_html_e( 'all done', 'seedcast-bulletin-library' );
							} else {
								printf(
									/* translators: %d: number of items missing this field. */
									esc_html( _n( '%d missing', '%d missing', (int) $count, 'seedcast-bulletin-library' ) ),
									(int) $count
								);
							}
							?>
			...
			<div class="sc-items__filters">
				...
				$labels['undated'] = __( 'No date', 'seedcast-bulletin-library' );
				$labels['all']     = __( 'All', 'seedcast-bulletin-library' );
			...
			<?php if ( empty( $page_of ) ) : ?>
				<p class="description"><?php esc_html_e( 'Nothing here.', 'seedcast-bulletin-library' ); ?></p>
			<?php else : ?>
				<table class="widefat striped">
					<thead>
						<tr>
							<th><?php echo esc_html( (string) $definition['label'] ); ?></th>
							<th style="width:190px;"><?php esc_html_e( 'Service week', 'seedcast-bulletin-library' ); ?></th>
							<th style="width:70px;"><?php esc_html_e( 'Score', 'seedcast-bulletin-library' ); ?></th>
							<th><?php esc_html_e( 'Missing', 'seedcast-bulletin-library' ); ?></th>
						</tr>
					</thead>
					<tbody>
							...
							<tr>
								...
								<td class="sc-item__missing">
									...
									echo empty( $item['missing'] )
										? esc_html__( 'Nothing', 'seedcast-bulletin-library' )
	...
	private static function render_tabs( string $current, array $definition, bool $show_comprehensive ): void {
		...
		<h2 class="nav-tab-wrapper sc-completeness__tabs">
			<a href="<?php echo esc_url( $base ); ?>"
				...
				<?php esc_html_e( 'Overview', 'seedcast-bulletin-library' ); ?>
			</a>
			...
			<?php if ( $show_comprehensive ) : ?>
				<a href="<?php echo esc_url( add_query_arg( 'seedcast_view', 'comprehensive', $base ) ); ?>"
					...
					<?php esc_html_e( 'Comprehensive score', 'seedcast-bulletin-library' ); ?>
				</a>
			<?php endif; ?>
		</h2>
		<?php
	...
	private static function render_comprehensive( string $source, array $archive, array $definition ): void {
		...
		<div class="sc-completeness">
			<p class="sc-completeness__explain">
				<?php
				esc_html_e( 'Your library score counts only the content you have chosen to use. The comprehensive score counts everything Sermon Library can track, whether you use it or not. It is the same measure on every church, which is what makes it worth comparing against.', 'seedcast-bulletin-library' );
				?>
			</p>
			...
			<div class="sc-completeness__compare">
				<div class="sc-score sc-score--<?php echo esc_attr( Completeness::band( $configured ) ); ?>">
					...
					<span class="sc-score__label"><?php esc_html_e( 'Your library', 'seedcast-bulletin-library' ); ?></span>
				</div>
				<div class="sc-score sc-score--<?php echo esc_attr( Completeness::band( $full ) ); ?>">
					...
					<span class="sc-score__label"><?php esc_html_e( 'Comprehensive', 'seedcast-bulletin-library' ); ?></span>
				</div>
			</div>

			<?php if ( ! empty( $excluded ) ) : ?>
				<h2><?php esc_html_e( 'Not currently in use', 'seedcast-bulletin-library' ); ?></h2>
				<p class="description">
					<?php esc_html_e( 'These are counted in the comprehensive score but not in your library score. You can switch any of them on in settings.', 'seedcast-bulletin-library' ); ?>
				</p>
		...
		<div class="sc-completeness">
			<div class="sc-completeness__hero">
				<div class="sc-score sc-score--<?php echo esc_attr( $band ); ?>">
					...
					<span class="sc-score__label"><?php esc_html_e( 'Library score', 'seedcast-bulletin-library' ); ?></span>
				</div>

				<div class="sc-completeness__stats">
					<div class="sc-stat">
						<span class="sc-stat__value"><?php echo esc_html( (string) $summary['average'] ); ?></span>
						<span class="sc-stat__label"><?php esc_html_e( '12 week average', 'seedcast-bulletin-library' ); ?></span>
					</div>
					<div class="sc-stat">
						<span class="sc-stat__value"><?php echo esc_html( (string) $summary['best'] ); ?></span>
						<span class="sc-stat__label"><?php esc_html_e( 'Best week', 'seedcast-bulletin-library' ); ?></span>
					</div>
						...
						<div class="sc-stat">
							<span class="sc-stat__value">
								...
								printf(
									...
									esc_html( _n( '%d day', '%d days', (int) $gap['median'], 'seedcast-bulletin-library' ) ),
									(int) $gap['median']
								);
								?>
							</span>
							<span class="sc-stat__label"><?php esc_html_e( 'Typical time to finish', 'seedcast-bulletin-library' ); ?></span>
						</div>
					<?php endif; ?>
					...
					<div class="sc-stat">
						<a class="sc-stat__link" href="<?php echo esc_url( add_query_arg( array( 'seedcast_view' => 'items', 'seedcast_filter' => 'complete' ), self::base_url() ) ); ?>">
							...
							<span class="sc-stat__label"><?php esc_html_e( 'Complete', 'seedcast-bulletin-library' ); ?></span>
						</a>
					</div>
				</div>
			</div>
			...
			<p class="sc-completeness__explain">
				...
				printf(
					...
					esc_html__( 'The library score is the average across all your sermons, not a count of finished ones. %1$d of %2$d are at 100%% so far, and the score rises whenever you fill anything in, including in bulk.', 'seedcast-bulletin-library' ),
			...
			<p class="sc-completeness__explain">
				<?php
				esc_html_e( 'The weekly scores below are a separate record of how each week actually went, and do not change once the week has passed.', 'seedcast-bulletin-library' );
				?>
			</p>
			...
			<div class="sc-completeness__buckets">
				<?php
				self::render_bar( __( 'Details', 'seedcast-bulletin-library' ), (int) $archive['metadata'] );
				self::render_bar( __( 'Written content', 'seedcast-bulletin-library' ), (int) $archive['content'] );
				?>
			</div>
	...
	private static function render_weeks( string $source, array $summary, string $start, array $archive = array() ): void {
		?>
		<h2><?php esc_html_e( 'Week by week', 'seedcast-bulletin-library' ); ?></h2>
		<?php
		/*
		...
		if ( ! empty( $archive['undated'] ) ) {
			$excluded[] = sprintf(
				...
				_n( '%d has no date', '%d have no date', (int) $archive['undated'], 'seedcast-bulletin-library' ),
				(int) $archive['undated']
			);
		}
		...
		if ( ! empty( $excluded ) ) :
			...
			<p class="description sc-completeness__excluded-note sc-completeness__excluded-note--warn">
				...
				printf(
					...
					esc_html__( 'Not counted in any week: %s. Sermons posted before you started using this report are also outside the weekly scores.', 'seedcast-bulletin-library' ),
				...
				<a href="<?php echo esc_url( add_query_arg( array( 'seedcast_view' => 'items', 'seedcast_filter' => 'undated' ), self::base_url() ) ); ?>">
					<?php esc_html_e( 'Show them', 'seedcast-bulletin-library' ); ?>
				</a>
			</p>
			<?php
		endif;
			...
			<p class="description">
				...
				if ( '' !== $start ) {
					printf(
						...
						esc_html__( 'Measurement began the week of %s. The first score appears once that week has finished.', 'seedcast-bulletin-library' ),
						esc_html( self::format_week( $start ) )
					);
				} else {
					esc_html_e( 'Nothing has been measured yet.', 'seedcast-bulletin-library' );
				}
				?>
			</p>
			<?php
				...
				<details class="sc-week sc-week--<?php echo esc_attr( $band ); ?>">
					<summary class="sc-week__summary">
						...
						<span class="sc-week__score">
							<?php echo $missing ? esc_html__( 'Nothing posted', 'seedcast-bulletin-library' ) : esc_html( $score . '%' ); ?>
						</span>
						<?php if ( ! empty( $week['is_amended'] ) ) : ?>
							<span class="sc-week__flag" title="<?php esc_attr_e( 'A date change after this week closed caused it to be recalculated.', 'seedcast-bulletin-library' ); ?>">
								<?php esc_html_e( 'amended', 'seedcast-bulletin-library' ); ?>
							</span>
						<?php endif; ?>
						<?php if ( empty( $week['is_locked'] ) ) : ?>
							<span class="sc-week__flag sc-week__flag--open"><?php esc_html_e( 'in progress', 'seedcast-bulletin-library' ); ?></span>
						<?php endif; ?>
					</summary>
					...
					<div class="sc-week__body">
						<?php if ( $missing ) : ?>
							<p class="description">
								<?php esc_html_e( 'No dated item fell in this week. A week with nothing posted counts as zero rather than being skipped.', 'seedcast-bulletin-library' ); ?>
							</p>
						<?php else : ?>
							<p class="sc-week__buckets">
								<span><?php esc_html_e( 'Details', 'seedcast-bulletin-library' ); ?>: <strong><?php echo esc_html( (string) (int) $week['metadata_score'] ); ?>%</strong></span>
								<span><?php esc_html_e( 'Written content', 'seedcast-bulletin-library' ); ?>: <strong><?php echo esc_html( (string) (int) $week['content_score'] ); ?>%</strong></span>
							</p>
							<table class="widefat striped sc-week__items">
								<thead>
									<tr>
										<th><?php esc_html_e( 'Item', 'seedcast-bulletin-library' ); ?></th>
										<th><?php esc_html_e( 'Score', 'seedcast-bulletin-library' ); ?></th>
										<th><?php esc_html_e( 'Missing', 'seedcast-bulletin-library' ); ?></th>
									</tr>
								</thead>
								<tbody>
						...
						<p class="description sc-week__version">
							...
							printf(
								...
								esc_html__( 'Scored against definition version %s.', 'seedcast-bulletin-library' ),
	...
	private static function render_definition( array $definition ): void {
		?>
		<h2><?php esc_html_e( 'What counts as complete', 'seedcast-bulletin-library' ); ?></h2>
		<p class="description">
			<?php
			printf(
				/* translators: %s: definition version. */
				esc_html__( 'Definition version %s. Weeks scored under an earlier version keep their original score and say so.', 'seedcast-bulletin-library' ),
		...
		<table class="widefat striped" style="max-width:640px;">
			<thead>
				<tr>
					<th><?php esc_html_e( 'Field', 'seedcast-bulletin-library' ); ?></th>
					<th><?php esc_html_e( 'Counts toward', 'seedcast-bulletin-library' ); ?></th>
				</tr>
			</thead>
			<tbody>
					...
					<tr>
						...
						<td>
							...
							echo ( isset( $field['bucket'] ) && 'content' === $field['bucket'] )
								? esc_html__( 'Written content', 'seedcast-bulletin-library' )
								: esc_html__( 'Details', 'seedcast-bulletin-library' );
							?>
						</td>
					</tr>
				<?php endforeach; ?>
	...
	private static function render_week_cell( array $item, array $weeks, string $start ): void {
		...
		if ( '' === $date ) {
			?>
			<span class="sc-item__nodate"><?php esc_html_e( 'No date', 'seedcast-bulletin-library' ); ?></span>
			<br />
			<span class="sc-item__weeknote"><?php esc_html_e( 'Counts toward no week', 'seedcast-bulletin-library' ); ?></span>
			<?php
			return;
		}
		...
		if ( '' !== $start && $week < $start ) {
			?>
			<span class="sc-item__weeknote"><?php esc_html_e( 'Before measuring began', 'seedcast-bulletin-library' ); ?></span>
			<?php
			return;
		}
		...
		if ( ! isset( $weeks[ $week ] ) ) {
			?>
			<span class="sc-item__weeknote sc-item__weeknote--open"><?php esc_html_e( 'This week, still open', 'seedcast-bulletin-library' ); ?></span>
			<?php
			return;
		}
		...
		printf(
			...
			esc_html(
				sprintf(
					...
					__( 'Week recorded at %d%%', 'seedcast-bulletin-library' ),
					(int) $row['score']
				)
			)
		);
		...
		if ( ! empty( $row['is_amended'] ) ) {
			?>
			<span class="sc-week__flag"><?php esc_html_e( 'amended', 'seedcast-bulletin-library' ); ?></span>
			<?php
		}
	}
