/*
 * Bulletin Library lightbox, for galleries in a service overview.
 *
 * WordPress's [gallery] links each image to its file or to its attachment
 * page and leaves the rest to the theme. Plenty of themes do nothing, so a
 * click leaves the service page for a bare image. This opens it in place
 * instead: previous and next through that gallery, the caption, arrow keys,
 * swipe, and Escape or a click outside the image to close.
 *
 * Built on <dialog>, which brings the focus trap, Escape and the top layer
 * with it. No jQuery and no library.
 */
( function () {
	'use strict';

	var strings   = window.scblLightbox || {};
	var IMAGE     = /\.(jpe?g|png|gif|webp|avif)(\?.*)?$/i;
	var galleries = document.querySelectorAll( '.scbl-single .gallery' );
	...
	// The largest file a thumbnail offers, for galleries that link to the
	// attachment page rather than to the image itself.
	...
	function collect( gallery ) {
		...
		gallery.querySelectorAll( '.gallery-item' ).forEach( function ( item ) {
			...
			list.push( {
				...
				// The service gallery keeps captions off its small thumbnails
				// and hands them over on the link instead.
	...
	function build() {
		...
		dialog.className = 'scbl-lightbox';
		dialog.setAttribute( 'aria-label', strings.label || 'Image viewer' );

		// Fixed markup only; every label and caption goes in as text below.
		dialog.innerHTML =
			'<figure class="scbl-lightbox__figure">' +
				'<img class="scbl-lightbox__img" alt="">' +
				'<figcaption class="scbl-lightbox__caption"></figcaption>' +
			'</figure>' +
			'<p class="scbl-lightbox__count" aria-live="polite"></p>' +
			'<button type="button" class="scbl-lightbox__prev"></button>' +
			'<button type="button" class="scbl-lightbox__next"></button>' +
			'<button type="button" class="scbl-lightbox__close" autofocus></button>';

		document.body.appendChild( dialog );

		img     = dialog.querySelector( '.scbl-lightbox__img' );
		caption = dialog.querySelector( '.scbl-lightbox__caption' );
		count   = dialog.querySelector( '.scbl-lightbox__count' );

		var prev  = dialog.querySelector( '.scbl-lightbox__prev' );
		var next  = dialog.querySelector( '.scbl-lightbox__next' );
		var close = dialog.querySelector( '.scbl-lightbox__close' );
		...
		// A click on the dim area around the image closes it.
		dialog.addEventListener( 'click', function ( e ) {
			if ( e.target === dialog || e.target.classList.contains( 'scbl-lightbox__figure' ) ) {
				closeViewer();
			}
		} );
	...
	// Unlock the page, drop the large image and give focus back to the
	// photo that opened the viewer. Safe to run twice.
	function cleanUp() {
		document.documentElement.classList.remove( 'scbl-lightbox-open' );
		if ( img ) {
	...
	function open( list, index, from ) {
		...
		show( index );
		document.documentElement.classList.add( 'scbl-lightbox-open' );
		dialog.showModal();
	}
	...
	galleries.forEach( function ( gallery ) {
		...
		list.forEach( function ( item, index ) {
			...
			if ( 'A' === target.tagName ) {
				...
				// An unlinked gallery image still opens, from the keyboard too.
